...
# Configuration
#------------------
NUM_TRIALS = 100  # Number of fault injection trials per starting layer per quadrant
SEED = 42
...
DISTRIBUTED_FAULT_PROBABILITIES = {
    2: 0.60,  # 60% chance of affecting 2 adjacent layers
    3: 0.20,  # 20% chance of affecting 3 adjacent layers
    4: 0.10,  # 10% chance of affecting 4 adjacent layers
    5: 0.10,  # 10% chance of affecting 5 adjacent layers
}
...
def sample_num_layers():
    """
    Sample number of adjacent layers to affect based on probability distribution.
    Returns: Number of layers to affect (2-5)
    """
...
#------------------
def inject_distributed_weight_faults(model, injectable_layers, starting_layer_idx, 
                                    quadrant_bits, num_layers):
    """
    Inject single bit flip in adjacent layers starting from starting_layer_idx.
    
    Args:
        model: The model
        injectable_layers: List of (name, module, weight_type) tuples
        starting_layer_idx: Index of first layer to inject fault
        quadrant_bits: Bit positions to choose from
        num_layers: Number of adjacent layers to affect
    
    Returns:
        List of fault info dictionaries
    """
    injected_faults = []
...
class DistributedActivationFaultInjector:
    """Hook-based distributed activation fault injector for adjacent layers."""
    ...
    def __init__(self, injectable_layers, starting_layer_idx, num_layers, quadrant_bits):
        ...
        # Determine which layers to affect (get unique base modules)
        unique_modules = []
        seen_names = set()
    ...
    def register_hooks(self):
        """Register forward hooks for all affected layers."""
...
def analyze_distributed_weight_sensitivity_by_quadrant(model_path, test_loader):
    """
    Analyze sensitivity to distributed weight faults across adjacent layers by bit quadrant.
    ...
    # Results storage: quadrant -> starting_layer -> list of MSE values
    quadrant_results = {}
    ...
    for quad_name, quad_bits in BIT_QUADRANTS.items():
        ...
        # For each possible starting layer
        for starting_layer_idx in range(len(injectable_layers)):
            ...
            for trial in tqdm(range(NUM_TRIALS), desc=f"  Trials"):
                ...
                # Re-fetch injectable layers after reload
                current_injectable_layers = get_injectable_layers(model)
                
                # Sample number of adjacent layers to affect
                ...
                injected_faults = inject_distributed_weight_faults(
                    model, current_injectable_layers, starting_layer_idx, 
                    quad_bits, num_layers)
            ...
            print(f"  Layer count dist: 2L={layer_count_distribution[2]}, "
                  f"3L={layer_count_distribution[3]}, "
                  f"4L={layer_count_distribution[4]}, "
                  f"5L={layer_count_distribution[5]}")
            if valid_count > 0:
...
def analyze_distributed_activation_sensitivity_by_quadrant(model_path, test_loader):
    """
    Analyze sensitivity to distributed activation faults across adjacent layers by bit quadrant.
    ...
    for quad_name, quad_bits in BIT_QUADRANTS.items():
        ...
        for starting_layer_idx in range(len(injectable_layers)):
            ...
            for trial in tqdm(range(NUM_TRIALS), desc=f"  Trials"):
                ...
                # Create distributed injector
                injector = DistributedActivationFaultInjector(
                    injectable_layers, starting_layer_idx, num_layers, quad_bits)
...
def plot_quadrant_comparison(quadrant_results, injectable_layers, fault_target, output_dir='plots'):
    """
    Plot layer sensitivity comparison across bit quadrants for distributed faults.
    """
    ...
    plt.suptitle(f'Distributed {fault_target.capitalize()} Fault Sensitivity by Bit Quadrant\n'
                 f'(Adjacent Layer Faults Starting from Each Layer)', 
...
def plot_real_vs_complex_comparison(quadrant_results, injectable_layers, output_dir='plots'):
    ...
    plt.suptitle('SpectralConv: Real vs Imaginary Component Sensitivity (Distributed Faults)', 
                 fontsize=16, fontweight='bold')
    plt.tight_layout()
...
def create_summary_dataframe(quadrant_results, injectable_layers, is_weight=True):
    """Create summary DataFrame for all quadrants with distributed fault information."""
    if is_weight:
        layer_names = [name for name, _, _ in injectable_layers]
    else:
        layer_names = [name for name, _, _ in injectable_layers]
...
if __name__ == "__main__":
    ...
    weight_results, injectable_layers = analyze_distributed_weight_sensitivity_by_quadrant(
        model_path, test_loader)
    
    # Create summary DataFrame
    df_weight_summary = create_summary_dataframe(weight_results, injectable_layers, is_weight=True)
    df_weight_summary.to_csv(os.path.join(output_dir, 'distributed_weight_quadrant_summary.csv'), 
                             index=False)
    ...
    activation_results, _ = analyze_distributed_activation_sensitivity_by_quadrant(
        model_path, test_loader)
    
    # Create summary DataFrame
    df_activation_summary = create_summary_dataframe(activation_results, injectable_layers, is_weight=False)
    df_activation_summary.to_csv(os.path.join(output_dir, 'distributed_activation_quadrant_summary.csv'), 
                                  index=False)
